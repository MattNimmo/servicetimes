...
- **Phase 0 — land ingestion + cleanup** (small, do first):
  merge `codex/recurring-ingestion`; apply the review trims (extract the shared
  Supabase REST helper — see Phase 1; collapse the duplicated per-campus
  fetch+build into one `buildCampusPlan(campus)`; drop the redundant verifier
  "open incident count" check; fix the test-file type errors); add a `typecheck`
  (`tsc --noEmit`) step to CI so test types can't rot.
...
- **Phase 3 — recommendations / levers.** Populate `plan_changes`
  (recommendation vs manual, approve/apply) from variance against
  `reference_target_seconds`, scoped to `is_lever_eligible` elements.
...
> Config gap to fix before "vs reference" is meaningful: every campus currently
> has the **default `reference_target_seconds = 4500`** (the deploy seed never
> overrides it), but e.g. MG runs ~90 min. Set per-campus reference targets
> (a tiny config migration) as part of Phase 1 or 2. The dashboard still renders
> correctly meanwhile; the reference delta is just not yet trustworthy.
...
- `src/proxy.ts` (Next 16 renamed `middleware.ts` -> `proxy.ts`; runs on Node
  ...
  `/`. Matcher excludes `_next/*`, `favicon.ico`, `/login`, and `/api/pco/*`
  (already `CRON_SECRET`-guarded).
...
`loginAction` constant-time-compares the submitted password against
`OPERATOR_PASSWORD` then `VIEWER_PASSWORD`, sets the cookie, redirects `/`. Wrong
password -> generic error (never reveal which role). `logoutAction` clears the
cookie. Config guard: if `AUTH_SESSION_SECRET`/passwords are unset or too short,
return a 503-style "auth not configured" (mirrors the ingest route's guard).
...
- **Slot grain**: already one row per `effective_plan_times`; the app computes
  `actual_service_seconds` vs `planned_target_seconds` vs campus
  `reference_target_seconds` in TypeScript. No view (it'd duplicate subtraction).
...
views). One row per plan / effective_slot / element_key, summing
`items.planned_seconds` and `item_times.actual_seconds`, with filters:
`items.is_rollup_child = false` (no double-count), `element_key is not null`,
`item_times.pco_exclude = false`, `sections.is_analytics_eligible = true`,
`elements.is_tracked = true`, `items.seen_in_last_pull = true`, joined to
`effective_plan_times` requiring `is_manually_excluded = false`,
`effective_slot_id is not null`, `time_type = 'service'`, grouped by
`effective_slot_id` (respects manual slot remaps). Include
`actual_is_complete = bool_and(item_times.actual_seconds is not null)`. Reads raw
actuals; a correction-aware `coalesce` is a deliberate later boundary.

### Decision 3: never fabricate precision

Three states, reused at both grains: `complete`, `needs_review` (actual null —
MG's null/zero live bounds — **or** covered by an open `review_incident`),
`no_plan`. A `needs_review` cell renders a pill, not a number, even if an actual
exists but the slot is under open review. Open-incident coverage = one read of
`review_incidents?status=eq.open` for the plan, matched in app code across both
incident shapes (`plan_time_id`-scoped and `plan_id`+`slot_id`-scoped).
...
- `/variance` — campus index (code, name, reference target).
...
- `/variance/[campus]/[serviceDate]` — the dashboard: header with two
  ...
  filtered by campus+date), per-slot planned/actual/reference + delta/% or
...
- Views `effective_plan_times`, `unmapped_items`; generated columns
  `planned_target_seconds`/`actual_service_seconds`/`actual_seconds`; campus
  `reference_target_seconds`; `sections.is_analytics_eligible`,
...
3. **Variance vs loaded 2026-06-21 data** (read-only REST queries + eyeball):
   - Slot deltas equal `actual_service_seconds - planned_target_seconds` and
     `- reference_target_seconds` for non-excluded service slots (SLP/ELK/LV).
   ...
   - **MG**: null/incomplete live bounds and open-incident-covered slots render
...
Corrections/incident resolution writes (Phase 2), recommendations (Phase 3),
per-campus reference-target tuning beyond the noted config fix, and any
