...
**Status:** ✅ Implemented (2026-07-01) — historical record. Shipped: Part 1B run-through exclusion (`6069c43`), 2.1 resolution toast + 2.2 real minus (`7861c3f`), 2.3 focus rings + 2.4 disabled date steppers + 2.5 labeled date picker (`59dcf33`), 2.6 responsive grid collapse + 2.7 display-name cleanup (`5a9143d`), 2.8 inline-style / hardcoded-color refactor (`5b90fb1`). Deferred optional product work: Part 1A "awaiting actuals" signal.
...
### Gap 1A — no "awaiting actuals" signal — ⬜ deferred *(optional product work; build only if requested)*
...
### Gap 1B — run-through plan_times count as "completed" — ✅ shipped (`6069c43`)
...
### 2.1 — Resolution toast never fires *(BUG)* — ✅ shipped (`7861c3f`)
...
### 2.2 — Cumulative time uses a hyphen, not the real minus *(BUG)* — ✅ shipped (`7861c3f`)
...
### 2.3 — No focus-visible styles in the instrument *(A11Y)* — ✅ shipped (`59dcf33`)
...
### 2.4 — Prev/Next date steppers aren't truly disabled *(A11Y)* — ✅ shipped (`59dcf33`)
...
### 2.5 — Date `<select>` unlabeled + visually inconsistent *(USABILITY)* — ✅ shipped (`59dcf33`)
...
### 2.6 — Service-order grid has no responsive collapse *(USABILITY)* — ✅ shipped (`5a9143d`)
...
### 2.7 — Mapped-row sub-note leaks the raw element key *(POLISH)* — ✅ shipped (`5a9143d`)
